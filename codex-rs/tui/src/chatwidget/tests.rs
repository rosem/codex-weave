...
#[tokio::test]
async fn build_user_input_items_adds_agent_mentions() {
    ...
    let mut mention_map = HashMap::new();
    mention_map.insert("writer".to_string(), thread_id);
    chat.set_agent_mention_map(mention_map);

    let message = UserMessage {
        text: "Hi #Writer and #Writer".to_string(),
        local_images: Vec::new(),
        text_elements: Vec::new(),
    };

    let items = chat.build_user_input_items(&message).expect("items");
    let expected = vec![
        UserInput::Text {
            text: "Hi #Writer and #Writer".to_string(),
            text_elements: Vec::new(),
