...
impl ChatWidget {
    ...
    fn weave_action_message_payload(
        ...
        reply_policy: &str,
        defer_until_action_id: Option<&str>,
    ) -> Value {
        let mut payload = serde_json::Map::new();
        ...
        if let Some(defer_until_action_id) = defer_until_action_id
            .map(str::trim)
            .filter(|value| !value.is_empty())
        {
            payload.insert(
                ...
                WeaveRelayAction::Message {
                    ...
                    for agent in &resolved {
                        ...
                        group.actions.push(Self::weave_action_message_payload(
                            ...
                            reply_to_action_id.as_deref(),
                            relay_reply_policy,
                            defer_until_action_id.as_deref(),
                        ));
                        self.register_pending_weave_action(
                            agent.id.as_str(),
                            action_id.as_str(),
    ...
    pub(crate) fn on_weave_message_received(&mut self, mut message: WeaveIncomingMessage) {
        ...
        {
            if self.should_defer_weave_action_message() {
    ...
    fn should_remap_pending_weave_message(
        ...
        !context.previous_conversation_id.is_empty()
