...
pub(crate) struct ChatWidget {
    ...
    // Local user messages that have already been rendered; used to dedupe live events.
    pending_user_message_signatures: VecDeque<UserMessageSignature>,
    // Mapping of mention aliases (lowercase) to thread ids for #Alias expansion.
    agent_mention_map: HashMap<String, ThreadId>,
...
struct UserMessageSignature {
    message: String,
    local_images: Vec<PathBuf>,
    text_elements: Vec<TextElement>,
}

impl UserMessageSignature {
...
#[derive(Debug)]
pub(crate) struct ChatDraftState {
    composer_text: String,
    composer_text_elements: Vec<TextElement>,
    composer_local_images: Vec<LocalImageAttachment>,
    queued_user_messages: VecDeque<UserMessage>,
}
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: codex_core::protocol::SessionConfiguredEvent) {
        self.app_event_tx.set_thread_id(event.session_id);
        self.bottom_pane
    ...
    pub(crate) fn set_agent_mentions(&mut self, aliases: Vec<String>) {
        self.bottom_pane.set_agent_mentions(aliases);
    }

    pub(crate) fn set_agent_mention_map(&mut self, map: HashMap<String, ThreadId>) {
        self.agent_mention_map = map;
    }
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let mut widget = Self {
            ...
            pending_user_message_signatures: VecDeque::new(),
            agent_mention_map: HashMap::new(),
    ...
    pub(crate) fn set_footer_right_label(&mut self, label: Option<Line<'static>>) {
        self.bottom_pane.set_footer_right_label(label);
    }
    ...
    pub(crate) fn open_agent_alias_prompt(&mut self) {
        let view = crate::bottom_pane::AgentAliasView::new(self.app_event_tx.clone());
        self.bottom_pane.show_view(Box::new(view));
        self.request_redraw();
    }
        ...
        match cmd {
            ...
            SlashCommand::TestApproval => {
                ...
                if let Some(thread_id) = self.thread_id {
                    self.app_event_tx
                        .send(AppEvent::CodexEventForThread { thread_id, event });
                } else {
            ...
            SlashCommand::Agent if !trimmed.is_empty() => {
                ...
                match action {
                    "new" => {
                        let alias = rest.trim();
                        if alias.is_empty() {
                            self.app_event_tx.send(AppEvent::OpenNewAgentPrompt);
                        } else {
                            self.app_event_tx.send(AppEvent::CreateIdleAgent {
                                alias: alias.to_string(),
                            });
                        }
                    }
                    "close" => {
                        self.app_event_tx.send(AppEvent::OpenCloseAgentPicker);
                    }
                    _ => {
                        self.dispatch_command(cmd);
                    }
    ...
    pub(crate) fn build_user_input_items(
        &self,
        user_message: &UserMessage,
    ) -> Option<Vec<UserInput>> {
        if user_message.text.is_empty() && user_message.local_images.is_empty() {
            return None;
        }
        ...
        if !user_message.text.is_empty() {
            ...
            items.extend(self.build_agent_mention_inputs(&user_message.text));
        }

        if let Some(skills) = self.bottom_pane.skills() {
            let skill_mentions = find_skill_mentions(&user_message.text, skills);
    ...
    fn build_agent_mention_inputs(&self, text: &str) -> Vec<UserInput> {
        if text.is_empty() || self.agent_mention_map.is_empty() {
            return Vec::new();
        }

        let aliases: Vec<String> = self.agent_mention_map.keys().cloned().collect();
        let matches = find_agent_alias_matches(text, &aliases);
        if matches.is_empty() {
            return Vec::new();
        }
        ...
        for matched in matches {
            let Some(thread_id) = self.agent_mention_map.get(&matched.alias_key) else {
                continue;
            };
            ...
            let alias = text
                .get(matched.alias_start..matched.alias_end)
                .unwrap_or(&matched.alias_key);
            mentions.push(UserInput::AgentMention {
                alias: alias.to_string(),
                thread_id: *thread_id,
            });
        }
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        self.submit_user_message_internal(user_message);
    }

    fn submit_user_message_internal(&mut self, user_message: UserMessage) {
        if !self.is_session_configured() {
        ...
        if let Some(stripped) = user_message.text.strip_prefix('!') {
        ...
        let Some(items) = self.build_user_input_items(&user_message) else {
            return;
        };
        ...
        self.track_local_user_message(&user_message);
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            EventMsg::ItemCompleted(ItemCompletedEvent { item, .. }) => {
                if let TurnItem::UserMessage(user) = item {
                    self.on_user_message_item_completed(user);
                }
            }
    ...
    fn track_local_user_message(&mut self, message: &UserMessage) {
        if message.text.trim().is_empty() {
            return;
        }
        self.pending_user_message_signatures
            .push_back(UserMessageSignature::from_user_message(message));
    }

    fn should_render_user_message(&mut self, event: &UserMessageEvent) -> bool {
        let signature = UserMessageSignature::from_event(event);
        if let Some(pos) = self
            .pending_user_message_signatures
            .iter()
            .position(|pending| pending == &signature)
        {
            self.pending_user_message_signatures.remove(pos);
            return false;
        }
        true
    }

    fn on_user_message_item_completed(&mut self, item: UserMessageItem) {
        let EventMsg::UserMessage(event) = item.as_legacy_event() else {
            return;
        };
        if self.should_render_user_message(&event) {
    ...
    pub(crate) fn take_draft_state(&mut self) -> ChatDraftState {
        ChatDraftState {
            composer_text: self.bottom_pane.composer_text(),
            composer_text_elements: self.bottom_pane.composer_text_elements(),
            composer_local_images: self.bottom_pane.composer_local_images(),
            queued_user_messages: std::mem::take(&mut self.queued_user_messages),
        }
    }

    pub(crate) fn restore_draft_state(&mut self, state: ChatDraftState) {
        let ChatDraftState {
            composer_text,
            composer_text_elements,
            composer_local_images,
            queued_user_messages,
        } = state;
        if !composer_text.is_empty()
            || !composer_text_elements.is_empty()
            || !composer_local_images.is_empty()
        {
            let local_image_paths = composer_local_images
                .into_iter()
                .map(|img| img.path)
                .collect();
            self.bottom_pane.set_composer_text(
...
#[derive(Debug)]
struct AgentAliasMatch {
    alias_key: String,
    alias_start: usize,
    alias_end: usize,
}

fn find_agent_alias_matches(text: &str, aliases: &[String]) -> Vec<AgentAliasMatch> {
    if aliases.is_empty() {
        return Vec::new();
    }

    let mut sorted_aliases: Vec<&String> = aliases.iter().collect();
    sorted_aliases.sort_by_key(|alias| std::cmp::Reverse(alias.len()));

    let mut matches = Vec::new();
    for (pos, _) in text.match_indices('#') {
        if pos > 0 {
            let before = text[..pos].chars().rev().next().unwrap_or(' ');
            if !is_agent_alias_boundary(before) {
                continue;
            }
        }
        let start = pos + 1;
        for alias in &sorted_aliases {
            ...
            if let Some(after) = after {
                if !is_agent_alias_boundary(after) {
                    continue;
                }
            }
            matches.push(AgentAliasMatch {
...
fn is_agent_alias_boundary(ch: char) -> bool {
